  ...
  return {
    ...
    async createJob() {
      if (!this.newJob.name.trim()) {
        LibreFangToast.warn('Please enter a job name');
        return;
      }
      if (!this.newJob.cron.trim()) {
        LibreFangToast.warn('Please enter a cron expression');
        return;
      }
      this.creating = true;
      try {
        ...
        LibreFangToast.success('Schedule "' + jobName + '" created');
        await this.loadJobs();
      } catch(e) {
        LibreFangToast.error('Failed to create schedule: ' + (e.message || e));
      }
      this.creating = false;
    },
    ...
    async toggleJob(job) {
      try {
        ...
        LibreFangToast.success('Schedule ' + (newState ? 'enabled' : 'paused'));
      } catch(e) {
        LibreFangToast.error('Failed to toggle schedule: ' + (e.message || e));
      }
    },
    ...
    deleteJob(job) {
      ...
      LibreFangToast.confirm('Delete Schedule', 'Delete "' + jobName + '"? This cannot be undone.', async function() {
        try {
          await LibreFangAPI.del('/api/cron/jobs/' + job.id);
          self.jobs = self.jobs.filter(function(j) { return j.id !== job.id; });
          LibreFangToast.success('Schedule "' + jobName + '" deleted');
        } catch(e) {
          LibreFangToast.error('Failed to delete schedule: ' + (e.message || e));
        }
      });
    },
    ...
    async runNow(job) {
      ...
      try {
        ...
        if (result.status === 'completed') {
          LibreFangToast.success('Schedule "' + (job.name || 'job') + '" executed successfully');
          job.last_run = new Date().toISOString();
        } else {
          LibreFangToast.error('Schedule run failed: ' + (result.error || 'Unknown error'));
        }
      } catch(e) {
        LibreFangToast.error('Run Now is not yet available for cron jobs');
      }
      this.runningJobId = '';
    },
    ...
    async toggleTrigger(trigger) {
      try {
        ...
        LibreFangToast.success('Trigger ' + (newState ? 'enabled' : 'disabled'));
      } catch(e) {
        LibreFangToast.error('Failed to toggle trigger: ' + (e.message || e));
      }
    },

    deleteTrigger(trigger) {
      var self = this;
      LibreFangToast.confirm('Delete Trigger', 'Delete this trigger? This cannot be undone.', async function() {
        try {
          await LibreFangAPI.del('/api/triggers/' + trigger.id);
          self.triggers = self.triggers.filter(function(t) { return t.id !== trigger.id; });
          LibreFangToast.success('Trigger deleted');
        } catch(e) {
          LibreFangToast.error('Failed to delete trigger: ' + (e.message || e));
        }
      });
    },
