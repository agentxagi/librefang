...
function channelsPage() {
  return {
    ...
    async startQR() {
      ...
      try {
        ...
        if (this.qr.connected) {
          LibreFangToast.success('WhatsApp connected!');
      ...
      this.qrPollTimer = setInterval(async function() {
        try {
          ...
          if (result.connected) {
            ...
            LibreFangToast.success('WhatsApp linked successfully!');
      ...
      try {
        ...
        try {
          ...
          if (testResult.status === 'ok') {
            ...
            this.setupStep = 3;
            LibreFangToast.success(this.setupModal.display_name + ' activated!');
          } else {
            LibreFangToast.success(this.setupModal.display_name + ' saved. ' + (testResult.message || ''));
          }
        } catch(te) {
          LibreFangToast.success(this.setupModal.display_name + ' saved. Test to verify connection.');
        }
        await this.refreshStatus();
      } catch(e) {
    ...
    async removeChannel() {
      ...
      var self = this;
      LibreFangToast.confirm('Remove Channel', 'Remove ' + displayName + ' configuration? This will deactivate the channel.', async function() {
        try {
          await LibreFangAPI.delete('/api/channels/' + name + '/configure');
          LibreFangToast.success(displayName + ' removed and deactivated.');
          await self.refreshStatus();
    ...
    async testChannel() {
      ...
      try {
        ...
        LibreFangToast.error('Test failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async copyConfig(ch) {
      ...
      try {
        ...
        LibreFangToast.success('Copied to clipboard');
      } catch(e) {
        LibreFangToast.error('Copy failed');
      }
    },
