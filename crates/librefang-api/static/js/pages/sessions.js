...
function sessionsPage() {
  return {
    ...
    deleteSession(sessionId) {
      var self = this;
      LibreFangToast.confirm('Delete Session', 'This will permanently remove the session and its messages.', async function() {
        try {
          await LibreFangAPI.del('/api/sessions/' + sessionId);
          self.sessions = self.sessions.filter(function(s) { return s.session_id !== sessionId; });
          LibreFangToast.success('Session deleted');
        } catch(e) {
          LibreFangToast.error('Failed to delete session: ' + e.message);
        }
      });
    },
    ...
    async addKey() {
      ...
      try {
        ...
        this.showAdd = false;
        LibreFangToast.success('Key "' + this.newKey + '" saved');
        this.newKey = '';
        this.newValue = '""';
        await this.loadKv();
      } catch(e) {
    ...
    deleteKey(key) {
      var self = this;
      LibreFangToast.confirm('Delete Key', 'Delete key "' + key + '"? This cannot be undone.', async function() {
        try {
          await LibreFangAPI.del('/api/memory/agents/' + self.memAgentId + '/kv/' + encodeURIComponent(key));
          LibreFangToast.success('Key "' + key + '" deleted');
          await self.loadKv();
        } catch(e) {
          LibreFangToast.error('Failed to delete key: ' + e.message);
        }
      });
    },
    ...
    async saveEdit() {
      ...
      try {
        ...
        LibreFangToast.success('Key "' + this.editingKey + '" updated');
        this.editingKey = null;
        this.editingValue = '';
        await this.loadKv();
      } catch(e) {
