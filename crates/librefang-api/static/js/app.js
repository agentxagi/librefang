...
document.addEventListener('alpine:init', function() {
  ...
  var savedKey = localStorage.getItem('librefang-api-key');
  if (savedKey) LibreFangAPI.setAuthToken(savedKey);
  ...
  Alpine.store('app', {
    ...
    toggleFocusMode() {
      ...
      localStorage.setItem('librefang-focus', this.focusMode);
    },
    ...
    async checkStatus() {
      try {
        var s = await LibreFangAPI.get('/api/status');
        this.connected = true;
        this.booting = false;
        this.lastError = '';
        ...
        console.warn('[LibreFang] Status check failed:', e.message);
      }
    },

    async checkOnboarding() {
      if (localStorage.getItem('librefang-onboarded')) return;
      try {
    ...
    dismissOnboarding() {
      ...
      localStorage.setItem('librefang-onboarded', 'true');
    },
    ...
    async checkAuth() {
      try {
        ...
        await LibreFangAPI.get('/api/tools');
        ...
        if (e.message && (e.message.indexOf('Not authorized') >= 0 || e.message.indexOf('401') >= 0 || e.message.indexOf('Missing Authorization') >= 0 || e.message.indexOf('Unauthorized') >= 0)) {
          ...
          var saved = localStorage.getItem('librefang-api-key');
          if (saved) {
    ...
    submitApiKey(key) {
      ...
      LibreFangAPI.setAuthToken(key.trim());
      localStorage.setItem('librefang-api-key', key.trim());
      this.showAuthPrompt = false;
      this.refreshAgents();
    },
...
function app() {
  return {
    page: 'agents',
    themeMode: localStorage.getItem('librefang-theme-mode') || 'system',
    theme: (() => {
      var mode = localStorage.getItem('librefang-theme-mode') || 'system';
      if (mode === 'system') return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
      return mode;
    })(),
    sidebarCollapsed: localStorage.getItem('librefang-sidebar') === 'collapsed',
    ...
    init() {
      ...
      LibreFangAPI.onConnectionChange(function(state) {
    ...
    setTheme(mode) {
      this.themeMode = mode;
      localStorage.setItem('librefang-theme-mode', mode);
      if (mode === 'system') {
    ...
    toggleSidebar() {
      ...
      localStorage.setItem('librefang-sidebar', this.sidebarCollapsed ? 'collapsed' : 'expanded');
    },
    ...
    async pollStatus() {
      ...
      this.wsConnected = LibreFangAPI.isWsConnected();
    }
  };
}
