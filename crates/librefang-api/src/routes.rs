        ...
        .await
    {
        Ok(resp) if resp.success => {
            if let Some(data) = &resp.data {
                ...
                if content.len() > 2000 {
                    content = format!("{}... (truncated)", librefang_types::truncate_str(&content, 2000));
                }
            }
        }
...
pub async fn set_provider_url(
    ...
    let probe =
        librefang_runtime::provider_health::probe_provider(&name, &base_url).await;
...
pub async fn serve_upload(Path(file_id): Path<String>) -> impl IntoResponse {
    ...
    let file_path = std::env::temp_dir().join("librefang_uploads").join(&file_id);
...
fn audit_to_comms_event(
    ...
    let (kind, detail, target_label) = match action_str.as_str() {
        "AgentMessage" => {
            ...
            let detail = if entry.detail.starts_with("tokens_in=") {
                ...
                if entry.outcome == "ok" {
                    ...
                    format!("{} in / {} out — {}", in_tok, out_tok, librefang_types::truncate_str(&entry.outcome, 80))
                }
            } else if entry.outcome != "ok" {
                format!("{} — {}", librefang_types::truncate_str(&entry.detail, 80), librefang_types::truncate_str(&entry.outcome, 80))
            } else {
        ...
        "AgentSpawn" => (
            ...
            format!("Agent spawned: {}", librefang_types::truncate_str(&entry.detail, 100)),
            "",
        ),
        "AgentKill" => (
            CommsEventKind::AgentTerminated,
            format!("Agent killed: {}", librefang_types::truncate_str(&entry.detail, 100)),
            "",
        ),
        _ => return None,
    };
