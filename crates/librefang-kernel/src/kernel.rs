...
pub struct LibreFangKernel {
    ...
    pub effective_mcp_servers: std::sync::RwLock<Vec<librefang_types::config::McpServerConfigEntry>>,
    ...
    pub channel_adapters: dashmap::DashMap<String, Arc<dyn librefang_channels::types::ChannelAdapter>>,
    /// Hot-reloadable default model override (set via config hot-reload, read at agent spawn).
    pub default_model_override: std::sync::RwLock<Option<librefang_types::config::DefaultModelConfig>>,
...
impl LibreFangKernel {
    ...
    pub fn boot_with_config(mut config: KernelConfig) -> KernelResult<Self> {
        ...
        let driver: Arc<dyn LlmDriver> = if driver_chain.len() > 1 {
            Arc::new(librefang_runtime::drivers::fallback::FallbackDriver::with_models(
                model_chain,
            ))
    ...
    pub async fn compact_agent_session(&self, agent_id: AgentId) -> KernelResult<String> {
        ...
        let (repaired_messages, repair_stats) =
            librefang_runtime::session_repair::validate_and_repair_with_stats(&result.kept_messages);
    ...
    pub fn start_background_agents(self: &Arc<Self>) {
                    ...
                    for job in due {
                        ...
                        match &job.action {
                            ...
                            librefang_types::scheduler::CronAction::AgentTurn {
                                ...
                                let kh: std::sync::Arc<dyn librefang_runtime::kernel_handle::KernelHandle> = kernel.clone();
        ...
        if let Some(ref a2a_config) = self.config.a2a {
            if a2a_config.enabled && !a2a_config.external_agents.is_empty() {
                ...
                tokio::spawn(async move {
                    let discovered = librefang_runtime::a2a::discover_external_agents(&agents).await;
    ...
    fn resolve_driver(&self, manifest: &AgentManifest) -> KernelResult<Arc<dyn LlmDriver>> {
        ...
        if !manifest.fallback_models.is_empty() {
            ...
            let mut chain: Vec<(std::sync::Arc<dyn librefang_runtime::llm_driver::LlmDriver>, String)> =
